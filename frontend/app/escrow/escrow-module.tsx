...
import {
  ...
  discoveryVerificationLabel,
  isClosedDiscoveryAgreement,
  roleForDiscoveryAgreement,
  roleLabel,
} from "./agreement-status";
import {
  deployAndFundAgreement,
  type DeployAndFundResult,
  type DeployAndFundStage,
} from "./deploy-and-fund-flow";
...
const REGISTRY = (process.env.NEXT_PUBLIC_EVIDENCE_REGISTRY_ADDRESS ??
...
export function EscrowModule({
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${API}/agreements/discovery`, {
        cache: "no-store",
      });
      if (!response.ok)
        throw new Error(
          `Unable to load live contracts (HTTP ${response.status}).`,
        );
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "Unable to load live contracts because the request failed.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const awaitingPayment = liveItems.filter(
    (item) => discoveryDisplayStatus(item) === "AwaitingPayment",
  ).length;
  const awaitingDelivery = liveItems.filter(
    (item) => discoveryDisplayStatus(item) === "AwaitingDelivery",
  ).length;
  const disputed = liveItems.filter(
    (item) => discoveryDisplayStatus(item) === "Disputed",
  ).length;
  if (creating)
    return (
      <EscrowWizard
        ...
        chainId={chainId}
        connector={connector as WalletConnector}
        close={() => {
          setCreating(false);
          void load();
        }}
  ...
  return (
    <div className="module-page">
      ...
      <div className="metric-grid">
        <Metric label="Live contracts" value={liveItems.length} />
        <Metric label="Awaiting payment" value={awaitingPayment} />
        <Metric label="Awaiting delivery" value={awaitingDelivery} />
        <Metric label="Closed" value={closedItems.length} />
        <Metric label="Disputed" value={disputed} tone="amber" />
      </div>
      <GlassCard className="agreement-table">
        <div className="table-title">
          <div>
            <h2>Live Contracts</h2>
            <p>
              Deployed agreements are publicly discoverable. Actions require the
              matching participant wallet.
            </p>
          </div>
          <StatusBadge>{liveItems.length} live</StatusBadge>
        </div>
        {loading ? (
          <div className="empty-agreements">
            <span className="escrow-icon">{"\u25c7"}</span>
            <h3>Loading live contracts...</h3>
          </div>
        ) : error ? (
          ...
          <div className="live-contract-grid">
            {liveItems.map((item) => (
              <AgreementCard
                item={item}
                walletAddress={walletAddress}
          ...
          <div className="empty-agreements">
            <span className="escrow-icon">{"\u25c7"}</span>
            <h3>No live contracts</h3>
            <p>
              Deployed agreements are visible to everyone; role actions require
              the matching wallet.
            </p>
      ...
      <GlassCard className="agreement-table closed-contracts">
        <div className="table-title">
          <div>
            <h2>Closed / Completed Contracts</h2>
            <p>Historical agreements are retained for audit and review.</p>
          </div>
          <StatusBadge tone="green">{closedItems.length} closed</StatusBadge>
        </div>
        {!loading && !error && closedItems.length ? (
          ...
          <div className="empty-agreements">
            <span className="escrow-icon">{"\u25c7"}</span>
            <h3>No closed contracts</h3>
            <p>
              Completed, refunded, and cancelled agreements will appear here.
            </p>
          </div>
        )}
      </GlassCard>
...
function AgreementCard({
  ...
  const status = discoveryDisplayStatus(item);
  return (
    <a className="live-contract-card" href={`/dashboard/${item.id}`}>
      <header>
        ...
        <div>
          <strong>
            {item.escrowAddress.slice(0, 10)}...{item.escrowAddress.slice(-6)}
          </strong>
          <small>
            {item.network} {"\u00b7"} {role ? roleLabel(role) : "Read only"}
          </small>
        </div>
        <StatusBadge tone={agreementStatusTone(status)}>{status}</StatusBadge>
      </header>
      ...
      <dl>
        <div>
          <dt>Participants</dt>
          <dd>{discoveryCounterparty(item, role)}</dd>
        </div>
        <div>
          <dt>Condition</dt>
          <dd>{discoveryConditionLabel(item)}</dd>
        </div>
        <div>
          <dt>Verification</dt>
          <dd>{discoveryVerificationLabel(item)}</dd>
        </div>
      </dl>
      <footer>
        <span>Updated {new Date(item.updatedAt).toLocaleDateString()}</span>
        <span className="live-cta">{discoveryAction(item, role)}</span>
      </footer>
    </a>
  );
}

function EscrowWizard({
  walletAddress,
  networkName,
  chainId,
  connector,
  ...
  const [flowStage, setFlowStage] = useState<DeployAndFundStage>("deploying");
  const [flowResult, setFlowResult] = useState<DeployAndFundResult>();
    ...
    return {
      ...
      policy: externalCondition
        ? {
            ...
            amount: toConditionUnits(
              form.conditionAmount,
              form.conditionDecimals,
            ),
  ...
  function next() {
    setError("");
    if (step === 1 && (!form.buyer || !form.seller || !form.arbitrator))
      return setError("Enter all participant wallet addresses");
    if (
      step === 2 &&
    ...
    if (step === 4) {
      ...
      if (
        activeDeliverables.some(
          (deliverable) => deliverable.evidenceRequirements.length === 0,
        )
      )
        return setError(
          "Add at least one evidence requirement to every active deliverable",
        );
      if (
        activeDeliverables.some((deliverable) =>
          deliverable.evidenceRequirements.some(
            (requirement) => !requirement.label.trim(),
          ),
        )
      )
        return setError("Enter a label for every evidence requirement");
    }
    ...
    try {
      ...
      const result = await deployAndFundAgreement({
        ...
        createAndDeploy: async (draftToDeploy) => {
          ...
          if (
            deployedAgreement.deploymentStatus !== "DEPLOYED" ||
            !deployedAgreement.escrowAddress
          )
            throw new Error("Deployment was not confirmed");
          setAgreementId(deployedAgreement.id);
          setDeployed(deployedAgreement.escrowAddress);
          setStep(agreementWizardSteps.length);
        ...
        fundEscrow: (details) =>
          fundEscrow({
            ...
            details,
            reconcile: async () => client.getAgreement(details.metadata.id),
          }),
      ...
      if (!result.funded)
        setError(
          result.transaction.error ??
            "Funding did not complete. The deployed agreement remains saved as AwaitingPayment.",
        );
    } catch (reason) {
      setError(
        reason instanceof Error
  ...
  return (
    <div className="module-page wizard-page">
      <SectionHeader
        eyebrow="NEW ESCROW"
        title={
          step === agreementWizardSteps.length && flowResult
            ? "Agreement deployed"
            : "Create Escrow"
        }
        action={
          <button className="text-button" onClick={close} disabled={deploying}>
            Close
          </button>
        }
      />
      <div className="wizard-layout">
        ...
        <GlassCard className="wizard-card">
          {step === 1 && (
            <WizardSection
              title="Who is part of this agreement?"
              copy="The connected wallet is set as buyer. Each role is committed immutably at deployment."
            >
              <Field
                label="Buyer wallet"
                name="buyer"
          ...
          {step === 2 && (
            <WizardSection
              title="Set the escrow payment"
              copy="ETH is held by the escrow contract until the agreement reaches a valid settlement state."
            >
              <div className="asset-select selected">
                <span className="asset-icon">Ξ</span>
                <div>
                  <strong>Ethereum</strong>
                  <small>Native asset · {networkName}</small>
                </div>
                <StatusBadge>Selected</StatusBadge>
              </div>
              <Field
                label="Required amount (ETH)"
                name="requiredAmountEth"
                value={form.requiredAmountEth}
                update={update}
                type="number"
              />
              <details>
            ...
            <div className="deploy-success">
              ...
              <h2>
                {flowResult?.funded
                  ...
                  : deploying
                    ? flowStage === "deploying"
                      ? "Step 1 of 2 \u2014 Deploying agreement"
                      : "Step 2 of 2 \u2014 Funding escrow"
                    : "Ready to deploy and fund"}
              </h2>
              <p>
                {flowResult?.funded
                  ? "Funding is confirmed. The authoritative contract state is AwaitingDelivery."
                  : deploying
                    ? "Approve the exact required amount from the connected buyer wallet."
                    : flowResult
                      ? "Funding did not complete. The deployed agreement remains saved and can be funded from Live Contracts."
                      : "The backend deploys the agreement first, then the connected buyer wallet funds the exact required amount."}
              </p>
              {deployed && <code>{deployed}</code>}
              {agreementId && !deploying && (
                <a
                  className="glass-button primary-button"
                  href={`/dashboard/${agreementId}`}
                >
                  {flowResult?.funded ? "View Live Contract" : "Fund Contract"}
                </a>
              )}
              {agreementId && !deploying && (
          ...
          <div className="wizard-actions">
            ...
            <GlassButton
              className="primary-button"
              disabled={
                step === agreementWizardSteps.length
                  ? deploying || !preview || Boolean(agreementId)
                  : false
              }
              onClick={
                step === agreementWizardSteps.length
...
function WizardSection({
  ...
  return (
    <div className="wizard-section">
      <span className="eyebrow">AGREEMENT SETUP</span>
      <h2>{title}</h2>
      <p>{copy}</p>
      {children}
    </div>
  ...
  return (
    ...
    >
          ...
          <div className="form-two">
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  update("sourceChainKey", event.currentTarget.value)
                }
              >
                <option value="1">Sepolia (Attestcoin key 1)</option>
              </select>
            </label>
            <label>
              Asset type
              <select
                ...
                onChange={(event) => updateAssetKind(event.currentTarget.value)}
              >
            ...
            <Field
              label="Sender (seller)"
              name="expectedSender"
              value={form.expectedSender || form.seller}
              update={update}
            />
            <Field
              label="Recipient (buyer)"
              name="expectedRecipient"
              value={form.expectedRecipient || form.buyer}
              update={update}
            />
            <Field
              label="Amount"
            ...
            <Field
              ...
              name="conditionDecimals"
              value={form.conditionDecimals}
              update={update}
              type="number"
            />
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  update("amountRule", event.currentTarget.value)
                }
              >
      ...
      <aside className="evidence-note">
        ...
        <span>
          Attestcoin and Creditcoin verify objective blockchain transactions.
          They do not verify photographs, files, websites, GitHub work, or
          physical delivery quality.
        </span>
      </aside>
    </WizardSection>
  );
    ...
    >
      <div className="review-grid">
        ...
        <Review
          ...
          value={preview?.agreement ?? "Invalid"}
        />
      </div>
      {external && (
        <div className="review-grid">
          ...
          <Review
            ...
            value={
              form.assetKind === "erc20" ? form.tokenContract : "Native asset"
            }
          />
          ...
          <Review
            label="Amount"
            value={`${form.conditionAmount} / ${form.conditionDecimals} decimals`}
          />
