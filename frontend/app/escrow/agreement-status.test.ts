...
function discoveryItem(
  ...
  return {
    ...
    status:
      state === "Complete" || state === "Refunded" || state === "Cancelled"
        ? "closed"
        : "live",
    lifecycle: "application_work_evidence",
  };
}
...
describe("live contract status", () => {
  ...
  it("shows participant-specific actions and counterparties", () => {
    expect(agreementAction(item("buyer", "AwaitingPayment"))).toBe(
      "Fund Contract",
    );
    expect(agreementAction(item("arbitrator", "Disputed"))).toBe(
      "Review Dispute",
    );
    expect(agreementAction(item("seller", "Complete"))).toBe("View History");
    expect(agreementAction(item("buyer"))).toBe("Open Agreement");
    expect(agreementCounterparty(item("buyer", "AwaitingPayment"))).toBe(
      "0x200000...000002",
    );
    expect(agreementCounterparty(item("seller", "AwaitingDelivery"))).toBe(
      "0x100000...000001",
    );
  });
  ...
  it("classifies public discovery items as live or closed", () => {
    ...
    expect(isClosedDiscoveryAgreement(discoveryItem("AwaitingPayment"))).toBe(
      false,
    );
    expect(isClosedDiscoveryAgreement(discoveryItem("AwaitingDelivery"))).toBe(
      false,
    );
  ...
  it("recognizes the production seller regardless of address casing", () => {
    const item = {
      ...discoveryItem("AwaitingDelivery"),
      seller: "0x4C9dE9AEFb29Fc33CCeFbd048b244aBEf251Db02",
    };
    expect(roleForDiscoveryAgreement(item, item.seller)).toBe("seller");
    expect(roleForDiscoveryAgreement(item, item.seller.toLowerCase())).toBe(
      "seller",
    );
    expect(roleForDiscoveryAgreement(item, item.buyer)).toBe("buyer");
  });
  ...
  it("shows read-only actions and counterparties for public discovery", () => {
    ...
    expect(discoveryAction(item, "buyer")).toBe("Open Escrow");
    expect(discoveryAction(discoveryItem("AwaitingPayment"), "buyer")).toBe(
      "Fund Contract",
    );
    expect(discoveryCounterparty(item, "buyer")).toBe("0x200000...000002");
    expect(discoveryCounterparty(item, undefined)).toBe(
      "0x100000...000001 / 0x200000...000002",
    );
  });

  it("maps blockchain-only discovery cards to verification lifecycle states", () => {
    const pending = {
      ...discoveryItem("AwaitingDelivery"),
      lifecycle: "blockchain_condition_only" as const,
      verificationStatus: "pending" as const,
    };
    const unlocked = {
      ...discoveryItem("Complete"),
      lifecycle: "blockchain_condition_only" as const,
