...
export function DashboardHome({ address, balance, networkName, sendOpen, setSendOpen, sendEth }: { address: string; balance: string; networkName: string; sendOpen: boolean; setSendOpen: (open: boolean) => void; sendEth: (recipient: string, amount: string) => Promise<string> }) {
  ...
  const [asset, setAsset] = useState<"ETH" | "USDC">("ETH"); const [review, setReview] = useState(false);
...
function Asset({ symbol, name, detail, amount }: { symbol: string; name: string; detail: string; amount: string }) { return <div className="asset-row"><span className="asset-icon">{symbol}</span><div><strong>{name}</strong><small>{detail}</small></div><b>{amount}</b></div>; }
function Modal({ title, close, children }: { title: string; close: () => void; children: React.ReactNode }) { return <div className="modal-backdrop" role="dialog" aria-modal="true"><GlassCard className="glass-modal"><header><h2>{title}</h2><button aria-label="Close" onClick={close}>×</button></header>{children}</GlassCard></div>; }
