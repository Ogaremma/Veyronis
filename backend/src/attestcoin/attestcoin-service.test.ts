...
async function setup() {
  ...
  const common = coder.encode(
    ...
    [tx.nonce, tx.gasLimit, tx.from, false, recipient, tx.value, tx.data],
  );
  const fields = coder.encode(
    [
      "uint64",
      "uint128",
      "uint128",
...
describe("AttestcoinService", () => {
  it("uses the native BlockProver precompile address", () => {
    expect(blockProver.BLOCK_PROVER_PRECOMPILE_ADDRESS).toBe(
  ...
  it("decodes the SDK transaction-plus-receipt leaf after verification", async () => {
    ...
    if (result.ok)
      expect(result.transaction.sourceTransactionHash).toBe(
