...
import {
  createAgreementHttpHandler,
  InMemoryRateLimiter,
} from "./agreement-http.js";
...
async function setup(limit = 20) {
  ...
  const server = createServer(
    createAgreementHttpHandler(service as never, {
      auth,
      appEnv: "local",
      creationLimiter: limiter(),
      confirmationLimiter: limiter(),
    }),
  );
  servers.push(server);
...
async function setupDashboardList() {
  ...
  const details = vi.fn(async (id: string, address: string) => {
    ...
    if (address === wrongWallet.address)
...
describe("agreement HTTP authorization", () => {
  ...
  it("returns 401 for both mutating routes without a session", async () => {
    ...
    expect(
      (
        await fetch(`${url}/agreements/${agreementId}/confirm`, {
          method: "POST",
        })
      ).status,
    ).toBe(401);
  ...
  it("returns the same public discovery list for participant and non-participant wallets", async () => {
    ...
    ).resolves.toEqual([items, items, items, items]);
  });
  ...
  it("keeps participant-only agreement details authorization-protected", async () => {
    ...
    const responses = await Promise.all(
      [buyerCookie, sellerCookie, arbitratorCookie, unrelatedCookie].map(
        (cookie) =>
  ...
  it("scopes GET /agreements to the authenticated participant", async () => {
    ...
    const spoofedAddressResponse = await fetch(
      `${url}/agreements?address=${arbitrator.address}`,
      {
        headers: { cookie: buyerCookie },
      },
    );
    ...
    await expect(
      Promise.all([
        buyerResponse.json(),
        sellerResponse.json(),
        arbitratorResponse.json(),
        unrelatedResponse.json(),
      ]),
    ).resolves.toEqual([
      [
        {
          metadata: expect.objectContaining({ id: agreementId }),
          role: "buyer",
        },
      ],
      [
  ...
  it("authenticates and scopes external condition access and submission", async () => {
    ...
    expect(
      (await fetch(`${url}/agreements/${agreementId}/condition`)).status,
    ).toBe(401);
    expect(
      (
        await fetch(`${url}/agreements/${agreementId}/condition/verify`, {
          method: "POST",
    ...
    for (const cookie of [buyerCookie, sellerCookie, arbitratorCookie]) {
      const response = await fetch(
        `${url}/agreements/${agreementId}/condition`,
        {
          headers: { cookie },
        },
      );
    ...
    expect(
      (
        await fetch(`${url}/agreements/${agreementId}/condition`, {
          headers: { cookie: unrelatedCookie },
        })
      ).status,
    ).toBe(403);
    ...
    const sellerResponse = await fetch(
      `${url}/agreements/${agreementId}/condition/verify`,
      {
        method: "POST",
        headers: { cookie: sellerCookie },
        body: JSON.stringify(transactionHash),
